...
foreach ($rResult as $aRow) {
    ...
    if (is_numeric($id_fornecedor) || $id_projeto) {
        $numberOutput = '<a href="' . admin_url('financeiro/contaspagar/' . $aRow['id']) . '" target="_blank">' . format_invoice_number($aRow['id']) . '</a>';
    } else {
        $numberOutput = '<a href="' . admin_url('financeiro/contaspagar/' . $aRow['id']) . '" onclick="init_contaspagar(' . $aRow['id'] . '); return false;">' . format_invoice_number($aRow['id']) . '</a>';
    }
