 ...
 */
$CI = &get_instance();
if (!option_exists(SYNGLIA_APP_API . '_enabled'))
    add_option(SYNGLIA_APP_API . '_enabled', 1);
...
function addExceptionForCSRF()
{
    ...
    if (file_exists($filePath) && is_writable($filePath)) {
        if (!strpos(file_get_contents($filePath), "synglia_app_api") !== false) {
            $configFile = fopen($filePath, "a") or die("Unable to open $filePath file!");
            $codeToWrite = 'if ($config["csrf_protection"] == true ' . PHP_EOL . ' && isset($_SERVER["REQUEST_URI"]) ' . PHP_EOL . ' && strpos($_SERVER["REQUEST_URI"], "synglia_app_api") !== false) { ' . PHP_EOL . ' $config["csrf_protection"] = false; ' . PHP_EOL . ' }';
