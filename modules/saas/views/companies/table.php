...
foreach ($rResult as $aRow) {
    ...
    $invoiceLink = admin_url('invoices/list_invoices/' . $invoice->id);
    $packageLink = $is_single_package ? admin_url(PERFEX_SAAS_MODULE_NAME . '/pricing') : admin_url(PERFEX_SAAS_MODULE_NAME . '/packages/edit/' . $invoice->{$package_id_col});
    ...
    for ($i = 0; $i < count($customFields); $i++) {
        ...
        if ($customFields[$i] == 'name') {
            ...
            if (!empty($invoice->name)) {
                $_data =  '<a href="' . $packageLink . '" target="_blank">' . $invoice->name . '</a>';
                $_data .= '<div class="row-options tw-ml-9">';
