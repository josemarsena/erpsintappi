				...
				function (event, value, input) {
					...
					if (property.htmlAttr) {
						...
						if (
							...
							element = Vvveb.ContentManager.setHtml(
								element,
								value
							);
						} else if (property.htmlAttr == "innerText") {
							element = Vvveb.ContentManager.setText(
								element,
								value
							);
						} else {
		...
		for (var i in component.properties) {
			...
			if (property.inputtype == SectionInput) {
				...
				if (Vvveb.preservePropertySections && section.length) {
					...
					componentsPanelSections[propertySection].append(
						property.input
					);
					section = componentsPanelSections[propertySection].find(
						'.section[data-section="' + property.key + '"]'
...
Vvveb.WysiwygEditor = {
	...
	edit: function (element) {
		...
		$("#wysiwyg-editor").removeClass("d-none");
	...
	destroy: function (element) {
		...
		$("#wysiwyg-editor").addClass("d-none");
		this.isActive = false;
...
Vvveb.Builder = {
		...
		self.frameBody.on("mousemove dragover touchmove", function (event) {
			if (
				...
				if (self.isResize) {
					...
					try {
						if (
							offset.top < y - halfHeight ||
							offset.left < x - halfWidth
						) {
						...
						if (self.designerMode) {
							var parentOffset = self.dragElement
								.offsetParent()
								.offset();
				...
				} // else //uncomment else to disable parent highlighting when dragging
				{
					...
					$("#highlight-name").html(
						self._getElementType(event.target)
					);
				}
			}
		});
			...
			if (self.isDragging) {
				...
				if (self.dragMoveMutation === false) {
					...
					if (self.component.afterDrop)
						self.dragElement = self.component.afterDrop(
							self.dragElement
						);
				}
					...
					_updateSelectBox = function (event) {
						...
						$("#select-box").css({
							...
							left:
								...
								self.selectPadding,
							width:
								self.texteditEl.outerWidth() +
								self.selectPadding * 2,
							height:
								self.texteditEl.outerHeight() +
								self.selectPadding * 2,
						});
					};
	...
	_initBox: function () {
		...
		$(".components-list li ol li", addSectionBox).on(
			"click",
			function (event) {
		...
		$(".sections-list li ol li", addSectionBox).on(
			"click",
			function (event) {
		...
		$(".drag-elements-sidepane ul > li > ol > li[data-drag-type]").on(
			...
			function (event) {
				...
				if (self.component.dragStart)
					self.dragElement = self.component.dragStart(
						self.dragElement
					);
	...
	setHtml: function (html) {
		...
		else window.FrameDocument.body.innerHTML = getTag(html, "body");
...
Vvveb.Gui = {
	...
	saveAjax: function () {
		...
		return Vvveb.Builder.saveAjax(url, null, null, saveUrl)
			...
			*/
			...
			.fail(function (data, text, errorThrown) {
	...
	newPage: function () {
		...
		newPageModal
			...
			.submit(function (e) {
				...
				// Customization
				data["url"] = window.themeBaseUrl + data["url"];
				name =
					data["title"] === "index"
						? data["folder"]
	...
	init: function (doc) {
		if (doc) {
			...
			for (let i = 0; i < doc.styleSheets.length; i++) {
				...
				if (
					_style.ownerNode.id &&
					_style.ownerNode.id == "vvvebjs-styles"
				) {
					style = _style;
					break;
				}
...
Vvveb.FileManager = {
	...
	renamePage: function (element, e, duplicate = false) {
		let page = element[0].dataset;
		let newfile = prompt(
			`Enter new file name for "${page.file}"`,
			page.file
		);
		let _self = this;
		if (newfile) {
	...
	addPage: function (name, data) {
		...
		if (data.folder) {
			if (
				...
				data.folderTitle =
					data.folder[0].toUpperCase() + data.folder.slice(1);
		...
		// Customization
		if (!this.tree.find('li[data-page="' + name + '"]').length)
...
Vvveb.Breadcrumb = {
	...
	loadBreadcrumb: function (element) {
		...
		while (currentElement.parentElement) {
			this.addElement(
				{
					name: Vvveb.Builder._getElementType(
						currentElement
					).toLowerCase(),
				},
				currentElement
