...
/** Load library */
$CI->load->library(PERFEX_SAAS_MODULE_NAME . '/cpanel_api');
...
if (!function_exists('perfex_saas_get_cpanel')) {
    ...
    function perfex_saas_get_cpanel()
    {
        ...
        $prefix = $user . '_' . PERFEX_SAAS_ROUTE_ID . '_';
...
hooks()->add_action('perfex_saas_module_tenant_deployed', function ($data) {
    ...
    if ($can_use_subdomain) {
        try {
            // Dont use createSubdomain() as it requires autossl
            $cpanel->createAddonDomain($subdomain, $slug, $root_domain, $root_dir);
        } catch (\Throwable $th) {
            log_message('error', $th->getMessage());
        }
    }
