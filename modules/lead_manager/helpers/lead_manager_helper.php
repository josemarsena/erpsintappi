    ...
    if (
        ...
        for ($i = 0; $i < count($_FILES[$index_name]['name']); $i++) {
            ...
            if (!empty($tmpFilePath) && $tmpFilePath != '') {
                if (
                    _gestioneincloud_upload_error($_FILES[$index_name]['error'][$i])
...
function handle_whatsapp_file_uploads($reciever_id, $sender_type)
{
    ...
    if (
        ...
        for ($i = 0; $i < count($_FILES['file']['name']); $i++) {
            if (_gestioneincloud_upload_error($_FILES['file']['error'][$i])) {
                $errors[$_FILES['file']['name'][$i]] = _gestioneincloud_upload_error($_FILES['file']['error'][$i]);
                continue;
            }
