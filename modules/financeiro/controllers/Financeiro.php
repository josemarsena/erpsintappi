...
class Financeiro extends AdminController
{
    ...
    public function transaction()
    {
        ...
        if($data['group'] == 'sales'){
            ...
            $data['count_invoice'] = $this->financeiro_model->count_invoice_not_convert_yet();
            $data['count_payment'] = $this->financeiro_model->count_payment_not_convert_yet();
            $data['invoices'] = $this->financeiro_model->get_data_invoices_for_select();
            $data['payment_modes'] = $this->payment_modes_model->get();
        }
        
        $data['accounts'] = $this->financeiro_model->get_accounts();
        $data['account_to_select'] = $this->financeiro_model->get_data_account_to_select();
    ...
    public function sales_table()
    {
        if ($this->input->is_ajax_request()) {
            ...
            if ($this->input->post('from_date')) {
                ...
                if (!$this->financeiro_model->check_format_date($from_date)) {
            ...
            if ($this->input->post('to_date')) {
                ...
                if (!$this->financeiro_model->check_format_date($to_date)) {
    ...
    public function chart_of_accounts(){
        ...
        $data['account_types'] = $this->financeiro_model->get_account_types();
        $data['detail_types'] = $this->financeiro_model->get_account_type_details();
        $data['accounts'] = $this->financeiro_model->get_accounts();
    ...
    public function setting()
    {
        ...
        if ($data['group'] == 'mapping_setup') {
            $data['items'] = $this->financeiro_model->get_items_not_yet_auto();
            $this->load->model('invoice_items_model');
            $data['_items'] = $this->invoice_items_model->get();
            $this->load->model('taxes_model');
            $data['_taxes'] = $this->taxes_model->get();
            $data['taxes'] = $this->financeiro_model->get_taxes_not_yet_auto();

            $this->load->model('expenses_model');
            $data['_categories'] = $this->expenses_model->get_category();
            $data['categories'] = $this->financeiro_model->get_expense_category_not_yet_auto();

            $this->load->model('payment_modes_model');
            $data['_payment_modes'] = $this->payment_modes_model->get();
            $data['payment_modes'] = $this->financeiro_model->get_payment_mode_not_yet_auto();
    ...
    public function update_general_setting(){
        ...
        $success = $this->financeiro_model->update_general_setting($data);
        if($success == true){
    ...
    public function update_automatic_conversion(){
        ...
        $data = $this->input->post();
        $success = $this->financeiro_model->update_automatic_conversion($data);
        if($success == true){
            $message = _l('updated_successfully', _l('setting'));
            set_alert('success', $message);
        }
    ...
    public function accounts_table()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $accounts = $this->financeiro_model->get_accounts();
            $account_types = $this->financeiro_model->get_account_types();
            $detail_types = $this->financeiro_model->get_account_type_details();
            ...
            $join         = [];
            $result       = $this->financeiro_model->get_account_data_tables($aColumns, $sIndexColumn, $sTable, $join, $where, ['number', 'description', 'balance_as_of', $debit, $credit, 'default_account']);
            $output  = $result['output'];
            $rResult = $result['rResult'];
    ...
    public function account()
    {
        ...
        if ($this->input->post()) {
            ...
            if ($data['id'] == '') {
                ...
                $success = $this->financeiro_model->add_account($data);
                if ($success) {
                ...
                $id = $data['id'];
                unset($data['id']);
                $success = $this->financeiro_model->update_account($data, $id);
                if ($success) {
    ...
    public function get_data_convert($id, $type){
        ...
        if($type == 'payment'){
            ...
            if($invoice->currency_name != $currency->name){
                $amount = $this->financeiro_model->currency_converter($invoice->currency_name, $currency->name, 1);
            ...
            if($expense->currency_data->name != $currency->name){
                $amount = $this->financeiro_model->currency_converter($expense->currency_data->name, $currency->name, 1);
                $_html = '';
                $edit_template = "";
                $edit_template .= render_input('edit_exchange_rate','exchange_rate', $amount, 'number');
                $edit_template .= "<div class='text-center mtop10'>";
            ...
            $banking = $this->financeiro_model->get_transaction_banking($id);
            $html = '<table class="table border table-striped no-margin">
                      <tbody>
                        <tr class="project-overview">
            ...
            if($invoice->items){
                ...
                foreach ($invoice->items as $value) {
                    $item = $this->financeiro_model->get_item_by_name($value['description']);
                    $item_id = 0;
                    if(isset($item->id)){
                        $item_id = $item->id;
                    }
                    ...
                    if($account_history){
                        ...
                        $item_automatic = $this->financeiro_model->get_item_automatic($item_id);
    ...
    public function convert(){
        ...
        $success = $this->financeiro_model->add_account_history($data);
        if ($success) {
    ...
    public function transfer(){
        ...
        $data['accounts'] = $this->financeiro_model->get_accounts('', 'find_in_set(account_type_id, "2,3,4,5,7,8,9,10")');
    ...
    public function add_transfer(){
        ...
        if($data['id'] == ''){
            ...
            $success = $this->financeiro_model->add_transfer($data);
            ...
            $id = $data['id'];
            unset($data['id']);
            $success = $this->financeiro_model->update_transfer($data, $id);
            if ($success === 'close_the_book') {
                $message = _l('has_closed_the_book');
            }elseif ($success) {
    ...
    public function journal_entry(){
        ...
        $data['accounts'] = $this->financeiro_model->get_accounts();
        $data['accounts_to_select'] = $this->financeiro_model->get_data_account_to_select();
    ...
    public function new_journal_entry($id = ''){
        if ($this->input->post()) {
            ...
            if($id == ''){
                ...
                $success = $this->financeiro_model->add_journal_entry($data);
                if ($success === 'close_the_book') {
                    $message = _l('has_closed_the_book');
                ...
                $success = $this->financeiro_model->update_journal_entry($data, $id);
                if ($success === 'close_the_book') {
                    $message = _l('has_closed_the_book');
                    set_alert('warning', _l('has_closed_the_book'));
                }elseif ($success) {
        ...
        if($id != ''){
            $data['journal_entry'] = $this->financeiro_model->get_journal_entry($id);
        }
        $this->load->model('currencies_model');
        $data['currency'] = $this->currencies_model->get_base_currency();
        $data['next_number'] = $this->financeiro_model->get_journal_entry_next_number();
        $data['title'] = _l('journal_entry');
        $data['account_to_select'] = $this->financeiro_model->get_data_account_to_select();
    ...
    public function delete_journal_entry($id)
    {
        ...
        $success = $this->financeiro_model->delete_journal_entry($id);
        $message = '';
        if ($success) {
            $message = _l('deleted');
            set_alert('success', $message);
    ...
    public function rp_business_snapshot(){
        ...
        $data['currency'] = $this->currencies_model->get_base_currency();
        $data['data_report'] = $this->financeiro_model->get_data_balance_sheet_summary([]);
        $this->load->view('report/includes/balance_sheet_summary', $data);
    }
                ...
                if (!empty($tmpFilePath) && $tmpFilePath != '') {
                    ...
                    if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                        ...
                        if(count($arr_insert) > 0){
                            $this->financeiro_model->insert_batch_banking($arr_insert);
                        }
    ...
    public function get_data_transfer($id){
        $transfer = $this->financeiro_model->get_transfer($id);
    ...
    public function delete_transfer($id)
    {
        ...
        $success = $this->financeiro_model->delete_transfer($id);
        $message = '';
        if ($success) {
    ...
    public function get_data_account($id){
        $account = $this->financeiro_model->get_accounts($id);
    ...
    public function delete_account($id)
    {
        ...
        $success = $this->financeiro_model->delete_account($id);
        $message = '';
    ...
    public function new_rule($id = ''){
        ...
        if ($this->input->post()) {
            $data                = $this->input->post();
            if($id == ''){
                $success = $this->financeiro_model->add_rule($data);
                if ($success) {
                    set_alert('success', _l('added_successfully', _l('banking_rule')));
                }
            }else{
                $success = $this->financeiro_model->update_rule($data, $id);
                if ($success) {
        ...
        if($id != ''){
            $data['rule'] = $this->financeiro_model->get_rule($id);
        }
        $this->load->model('currencies_model');
    ...
    public function delete_convert($id,$type)
    {
        ...
        $success = $this->financeiro_model->delete_convert($id,$type);
    ...
    public function reconcile(){
        ...
        if ($this->input->post()) {
            ...
            if($data['resume'] == 0){
                unset($data['resume']);
                $success = $this->financeiro_model->add_reconcile($data);
            }
        ...
        $reconcile = $this->financeiro_model->get_reconcile_by_account($data['accounts'][0]['id']);
        if($reconcile){
    ...
    public function reconcile_account($account){
        ...
        $data['accounts'] = $this->financeiro_model->get_accounts();
        $data['account'] = $this->financeiro_model->get_accounts($account);
        $data['reconcile'] = $this->financeiro_model->get_reconcile_by_account($account);
        $this->load->model('currencies_model');
        $data['currency'] = $this->currencies_model->get_base_currency();
        $data['title'] = _l('reconcile');
    ...
    public function get_info_reconcile($account){
        $reconcile = $this->financeiro_model->get_reconcile_by_account($account);
        $beginning_balance = 0;
    ...
    public function adjustment()
    {
        ...
        if ($this->input->post()) {
            $data = $this->input->post();
            $message = '';
            $success = $this->financeiro_model->add_adjustment($data);
    ...
    public function finish_reconcile_account(){
        ...
        if ($this->input->post()) {
            $data = $this->input->post();
            $message = '';
            $success = $this->financeiro_model->finish_reconcile_account($data);
    ...
    public function edit_reconcile(){
        ...
        if ($this->input->post()) {
            ...
            $message = '';
            $success = $this->financeiro_model->update_reconcile($data, $id);
    ...
    public function delete_rule($id)
    {
        ...
        $success = $this->financeiro_model->delete_rule($id);
        $message = '';
        if ($success) {
            $message = _l('deleted');
            set_alert('success', $message);
    ...
    public function view_report(){
        ...
        switch ($data_filter['type']) {
            case 'balance_sheet':
                    $data['data_report'] = $this->financeiro_model->get_data_balance_sheet($data_filter);
                break;
            case 'balance_sheet_comparison':
                    $data['data_report'] = $this->financeiro_model->get_data_balance_sheet_comparison($data_filter);
                break;
            case 'balance_sheet_detail':
                    $data['data_report'] = $this->financeiro_model->get_data_balance_sheet_detail($data_filter);
                break;
            case 'balance_sheet_summary':
                    $data['data_report'] = $this->financeiro_model->get_data_balance_sheet_summary($data_filter);
                break;
            case 'custom_summary_report':
                    $data['data_report'] = $this->financeiro_model->get_data_custom_summary_report($data_filter);
                break;
            case 'profit_and_loss_as_of_total_income':
                    $data['data_report'] = $this->financeiro_model->get_data_profit_and_loss_as_of_total_income($data_filter);
                break;
            case 'profit_and_loss_comparison':
                    $data['data_report'] = $this->financeiro_model->get_data_profit_and_loss_comparison($data_filter);
                break;
            case 'profit_and_loss_detail':
                    $data['data_report'] = $this->financeiro_model->get_data_profit_and_loss_detail($data_filter);
                break;
            case 'profit_and_loss_year_to_date_comparison':
                    $data['data_report'] = $this->financeiro_model->get_data_profit_and_loss_year_to_date_comparison($data_filter);
                break;
            case 'profit_and_loss':
                    $data['data_report'] = $this->financeiro_model->get_data_profit_and_loss($data_filter);
                break;
            case 'statement_of_cash_flows':
                    $data['data_report'] = $this->financeiro_model->get_data_statement_of_cash_flows($data_filter);
                break;
            case 'statement_of_changes_in_equity':
                    $data['data_report'] = $this->financeiro_model->get_data_statement_of_changes_in_equity($data_filter);
                break;
            case 'deposit_detail':
                    $data['data_report'] = $this->financeiro_model->get_data_deposit_detail($data_filter);
                break;
            case 'income_by_customer_summary':
                    $data['data_report'] = $this->financeiro_model->get_data_income_by_customer_summary($data_filter);
                break;
            case 'check_detail':
                    $data['data_report'] = $this->financeiro_model->get_data_check_detail($data_filter);
                break;
            case 'general_ledger':
                    $data['data_report'] = $this->financeiro_model->get_data_general_ledger($data_filter);
                break;
            case 'journal':
                    $data['data_report'] = $this->financeiro_model->get_data_journal($data_filter);
                break;
            case 'recent_transactions':
                    $data['data_report'] = $this->financeiro_model->get_data_recent_transactions($data_filter);
                break;
            case 'transaction_detail_by_account':
                    $data['data_report'] = $this->financeiro_model->get_data_transaction_detail_by_account($data_filter);
                break;
            case 'transaction_list_by_date':
                    $data['data_report'] = $this->financeiro_model->get_data_transaction_list_by_date($data_filter);
                break;
            case 'trial_balance':
                    $data['data_report'] = $this->financeiro_model->get_data_trial_balance($data_filter);
                break;
            case 'account_history':
                    $data['data_report'] = $this->financeiro_model->get_data_account_history($data_filter);
                break;
            case 'tax_detail_report':
                    $data['data_report'] = $this->financeiro_model->get_data_tax_detail_report($data_filter);
                break;
            case 'tax_summary_report':
                    $data['data_report'] = $this->financeiro_model->get_data_tax_summary_report($data_filter);
                break;
            case 'tax_liability_report':
                    $data['data_report'] = $this->financeiro_model->get_data_tax_liability_report($data_filter);
                break;
            case 'account_list':
                    $data['data_report'] = $this->financeiro_model->get_data_account_list($data_filter);
                break;
            default:
                break;
        }
    ...
    public function get_data_dashboard(){
        ...
        $data['profit_and_loss_chart'] = $this->financeiro_model->get_data_profit_and_loss_chart($data_filter);
        $data['expenses_chart'] = $this->financeiro_model->get_data_expenses_chart($data_filter);
        $data['income_chart'] = $this->financeiro_model->get_data_income_chart($data_filter);
        $data['sales_chart'] = $this->financeiro_model->get_data_sales_chart($data_filter);
        $data['bank_accounts'] = $this->financeiro_model->get_data_bank_accounts_dashboard($data_filter);
        $data['convert_status'] = $this->financeiro_model->get_data_convert_status_dashboard($data_filter);
    ...
    public function reset_data(){
        ...
        $data = $this->input->post();
        $success = $this->financeiro_model->reset_data();
        if($success == true){
    ...
    public function change_account_status($id, $status)
    {
        if (has_permission('accounting_chart_of_accounts', '', 'edit')) {
            if ($this->input->is_ajax_request()) {
                $this->financeiro_model->change_account_status($id, $status);
            }
        }
    }
    ...
    public function item_automatic(){
        ...
        if($data['id'] == ''){
            ...
            $success = $this->financeiro_model->add_item_automatic($data);
            if($success){
            ...
            $id = $data['id'];
            unset($data['id']);
            $success = $this->financeiro_model->update_item_automatic($data, $id);
            $message = _l('fail');
            if ($success) {
    ...
    public function delete_item_automatic($id)
    {
        ...
        $success = $this->financeiro_model->delete_item_automatic($id);
        $message = '';
        if ($success) {
                ...
                if($type == 'payment'){
                    foreach ($ids as $id) {
                        if ($this->input->post('mass_convert') === 'true') {
                            if (has_permission('accounting_transaction', '', 'create')) {
                                if ($this->financeiro_model->automatic_payment_conversion($id)) {
                                    $total_deleted++;
                                }
                            }
                        }elseif($this->input->post('mass_delete_convert') === 'true'){
                            if (has_permission('accounting_transaction', '', 'delete')) {
                                if ($this->financeiro_model->delete_convert($id, 'payment')) {
                                    $total_deleted++;
                                }
                            }
                        }
                    ...
                    foreach ($ids as $id) {
                        if ($this->input->post('mass_convert') === 'true') {
                            if (has_permission('accounting_transaction', '', 'create')) {
                                if ($this->financeiro_model->automatic_invoice_conversion($id)) {
                                    $total_deleted++;
                                }
                            }
                        }elseif($this->input->post('mass_delete_convert') === 'true'){
                            if (has_permission('accounting_transaction', '', 'delete')) {
                                if ($this->financeiro_model->delete_convert($id, 'invoice')) {
                                    $total_deleted++;
                                }
                            }
                        }
                    ...
                    foreach ($ids as $id) {
                        if ($this->input->post('mass_convert') === 'true') {
                            if (has_permission('accounting_transaction', '', 'create')) {
                                if ($this->financeiro_model->automatic_expense_conversion($id)) {
                                    $total_deleted++;
                                }
                            }
                        }elseif($this->input->post('mass_delete_convert') === 'true'){
                            if (has_permission('accounting_transaction', '', 'delete')) {
                                if ($this->financeiro_model->delete_convert($id, 'expense')) {
                                    $total_deleted++;
                                }
                            }
                        }
                    ...
                    foreach ($ids as $id) {
                        if ($this->input->post('mass_delete') === 'true') {
                            if (has_permission('accounting_transaction', '', 'create')) {
                                if ($this->financeiro_model->delete_banking($id)) {
                                    $total_deleted++;
                                }
                            }
                        }elseif($this->input->post('mass_delete_convert') === 'true'){
                            if (has_permission('accounting_transaction', '', 'delete')) {
                                if ($this->financeiro_model->delete_convert($id, 'banking')) {
                                    $total_deleted++;
                                }
                            }
                        }
        ...
        if ($this->input->post()) {
            ...
            if (is_array($ids)) {
                foreach ($ids as $id) {
                    if($this->input->post('mass_delete') === 'true'){
                        if (has_permission('accounting_journal_entry', '', 'delete')) {
                            if ($this->financeiro_model->delete_journal_entry($id)) {
                                $total_deleted++;
                            }
                        }
                    }
        ...
        if ($this->input->post()) {
            ...
            if (is_array($ids)) {
                foreach ($ids as $id) {
                    if($this->input->post('mass_delete') === 'true'){
                        if (has_permission('accounting_transfer', '', 'delete')) {
                            if ($this->financeiro_model->delete_transfer($id)) {
                                $total_deleted++;
                            }
                        }
                    }
    ...
    public function tax_mapping(){
        ...
        if($data['id'] == ''){
            ...
            $success = $this->financeiro_model->add_tax_mapping($data);
            if($success){
            ...
            $id = $data['id'];
            unset($data['id']);
            $success = $this->financeiro_model->update_tax_mapping($data, $id);
            $message = _l('fail');
            if ($success) {
    ...
    public function delete_tax_mapping($id)
    {
        ...
        $success = $this->financeiro_model->delete_tax_mapping($id);
        $message = '';
        if ($success) {
            ...
            if (is_array($ids)) {
                foreach ($ids as $id) {
                    if($this->input->post('mass_delete') === 'true'){
                        ...
                        if (has_permission('accounting_chart_of_accounts', '', 'edit')) {
                            if ($this->financeiro_model->change_account_status($id, 1)) {
                                $total_deleted++;
                            }
                        }
                    }elseif($this->input->post('mass_deactivate') === 'true'){
                        if (has_permission('accounting_chart_of_accounts', '', 'edit')) {
                            if ($this->financeiro_model->change_account_status($id, 0)) {
                                $total_deleted++;
                            }
                        }
                    }
    ...
    public function expense_category_mapping(){
        ...
        if($data['id'] == ''){
            ...
            $success = $this->financeiro_model->add_expense_category_mapping($data);
            if($success){
            ...
            $id = $data['id'];
            unset($data['id']);
            $success = $this->financeiro_model->update_expense_category_mapping($data, $id);
            $message = _l('fail');
            if ($success) {
    ...
    public function delete_expense_category_mapping($id)
    {
        ...
        $success = $this->financeiro_model->delete_expense_category_mapping($id);
        $message = '';
        if ($success) {
    ...
    public function payment_mode_mapping(){
        ...
        if($data['id'] == ''){
            ...
            $success = $this->financeiro_model->add_payment_mode_mapping($data);
            if($success){
            ...
            $id = $data['id'];
            unset($data['id']);
            $success = $this->financeiro_model->update_payment_mode_mapping($data, $id);
            $message = _l('fail');
            if ($success) {
    ...
    public function delete_payment_mode_mapping($id)
    {
        ...
        $success = $this->financeiro_model->delete_payment_mode_mapping($id);
        $message = '';
        if ($success) {
    ...
    public function change_active_payment_mode_mapping($id, $status)
    {
        if (has_permission('accounting_setting', '', 'edit')) {
            if ($this->input->is_ajax_request()) {
                $this->financeiro_model->change_active_payment_mode_mapping($status);
            }
        }
    }
    ...
    public function change_active_expense_category_mapping($id, $status)
    {
        if (has_permission('accounting_setting', '', 'edit')) {
            if ($this->input->is_ajax_request()) {
                $this->financeiro_model->change_active_expense_category_mapping($status);
            }
        }
    }
    ...
    public function account_type_detail()
    {
        ...
        if ($this->input->post()) {
            ...
            if ($data['id'] == '') {
                ...
                $success = $this->financeiro_model->add_account_type_detail($data);
                if ($success) {
                ...
                $id = $data['id'];
                unset($data['id']);
                $success = $this->financeiro_model->update_account_type_detail($data, $id);
                if ($success) {
    ...
    public function delete_account_type_detail($id)
    {
        ...
        $success = $this->financeiro_model->delete_account_type_detail($id);
        $message = '';
    ...
    public function get_data_account_type_detail($id){
        $account_type_detail = $this->financeiro_model->get_data_account_type_details($id);
    ...
    public function journal_entry_export($id){
        ...
        $journal_entry = $this->financeiro_model->get_journal_entry($id);
    ...
    public function change_preferred_payment_method($id, $status)
    {
        if (has_permission('staff', '', 'edit')) {
            if ($this->input->is_ajax_request()) {
                $this->financeiro_model->change_preferred_payment_method($id, $status);
            }
        }
    }
}